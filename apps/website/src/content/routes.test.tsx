...
import { allBaseRoutes, type BaseRoute, generateRouteForLanguage, type Route } from "./routes";
...
describe("Route Validation", () => {
  ...
  const existingSlugs = astroFiles.map((file) => filePathToSlug(file, pagesDir));
  ...
  const uniqueRouteSlugs = [...new Set(routeSlugs)].filter((slug) => slug.length > 0);

  describe("Route Coverage", () => {
    it.skip("should have all route slugs covered by page files", () => {
        ...
        for (const language of languages) {
          if (language.code !== "en") {
            possibleFiles.push(
              ...
              path.join(pagesDir, language.code, slug, "index.astro"),
            );
          }
        }
    ...
    it.skip("should not have orphaned page files", () => {
  ...
  describe("Language-Specific Routes", () => {
    it.skip("should have language-specific pages for all non-English routes", () => {
      const nonEnglishRoutes = allRoutes.filter((route) => route.lang !== "en");
  ...
  describe("Route Structure", () => {
    ...
    it.skip("should have unique hrefs", () => {
  ...
  describe("FAQ Routes", () => {
    it("should have all FAQ routes with proper structure", () => {
      const faqRoutes = allBaseRoutes.filter((route) => route.slug.startsWith("FAQs/"));
      ...
      for (const faqRoute of faqRoutes) {
        ...
        for (const language of languages) {
          expect(faqRoute.slugTranslations![language.code]).toBeDefined();
          expect(typeof faqRoute.slugTranslations![language.code]).toBe("string");
          expect(faqRoute.slugTranslations![language.code].length).toBeGreaterThan(0);
        }
      }
    });
  });
  ...
  describe("Product Routes", () => {
    it("should have all product routes with proper structure", () => {
      const productRoutes = allBaseRoutes.filter(
        (route) => route.slug === "achilles-rupture-splint" || route.slug === "reviews",
      );
      ...
      for (const productRoute of productRoutes) {
        ...
        // Check that all languages have translations
        for (const language of languages) {
          expect(productRoute.slugTranslations![language.code]).toBeDefined();
          expect(typeof productRoute.slugTranslations![language.code]).toBe("string");
          expect(productRoute.slugTranslations![language.code].length).toBeGreaterThan(0);
        }
      }
    });
  });
