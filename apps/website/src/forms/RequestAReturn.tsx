...
export default function RequestAReturn({ initialStage = "info" }: RequestAReturnProps) {
  ...
  const canProceedToReason = () => {
    ...
    const isCountryValid = country && (country === "uk" || country === "usa");

    return hasRequiredFields && isEmailValid && isOrderIdValid && isCountryValid;
  };

  const canProceedToDetails = () => {
    return formData.returnReason && formData.returnReason !== "not-approved";
  };
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      formDataToSend.append("country", data.country === "uk" ? "United Kingdom" : "United States");
      ...
      if (data.returnReason === "uncomfortable") {
        formDataToSend.append("comfort-issues", (data.comfortIssues || []).join(", "));
        if (data.otherComfortReason) {
          formDataToSend.append("other-comfort-reason", data.otherComfortReason);
        }
      } else if (data.returnReason === "damaged") {
        formDataToSend.append("damage-description", data.damageDescription || "");
      } else if (data.returnReason === "expensive") {
        formDataToSend.append("acceptable-price", data.acceptablePrice || "");
      ...
      setSubmitError("Failed to submit your return request. Please try again.");
  ...
  const renderInfoStage = () => (
    <div className="space-y-6">
        ...
        render={({ field }) => (
          <FormItem>
            ...
            <Select onValueChange={field.onChange} value={field.value}>
              ...
              <SelectContent>
                <SelectItem value="uk" className="py-3 text-base">
                  <span className="pl-6">United Kingdom</span>
                </SelectItem>
                <SelectItem value="usa" className="py-3 text-base">
      ...
      <Button
        onClick={async () => {
          const isValid = await form.trigger(["email", "orderId", "country"]);
          if (isValid) {
            setStage("reason");
          }
        }}
  ...
  const renderReasonStage = () => (
    <div className="space-y-6">
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel className="text-lg">Reason for Return</FormLabel>
            <Select onValueChange={field.onChange} value={field.value}>
              <FormControl>
                <SelectTrigger className="text-base">
              ...
              <SelectContent>
                <SelectItem value="uncomfortable" className="py-3 text-base">
                  <span className="pl-6">The splint was uncomfortable</span>
                </SelectItem>
                <SelectItem value="damaged" className="py-3 text-base">
                  <span className="pl-6">The splint was damaged</span>
                </SelectItem>
                <SelectItem value="not-approved" className="py-3 text-base">
                  <span className="pl-6">The splint was not approved by my care team</span>
                </SelectItem>
                <SelectItem value="expensive" className="py-3 text-base">
                  <span className="pl-6">The splint was too expensive</span>
                </SelectItem>
                <SelectItem value="other" className="py-3 text-base">
      ...
      {formData.returnReason === "not-approved" && (
        <Alert className="flex bg-red-50 border-red-200">
          ...
          <AlertDescription className="text-red-800 text-lg">
            We always recommend checking with your care team before purchasing. Unfortunately, we
            cannot accept returns when the product was not approved by your healthcare provider.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="flex gap-2">
        ...
        {formData.returnReason && formData.returnReason !== "not-approved" && (
          <Button
    ...
    <div className="space-y-6">
      {formData.returnReason === "uncomfortable" && (
        <div className="space-y-4">
          <Label className="text-lg">What made the splint uncomfortable?</Label>
          <div className="gap-3 grid grid-cols-1 md:grid-cols-2">
            {[
              ...
              <div key={issue} className="flex items-center space-x-2">
                <Checkbox
                  id={issue}
                  checked={(formData.comfortIssues || []).includes(issue)}
                  onCheckedChange={() => handleComfortIssueToggle(issue)}
                />
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-lg">Other reason (optional)</FormLabel>
                <FormControl>
                  <Input placeholder="Please describe any other comfort issues..." {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
      ...
      {formData.returnReason === "damaged" && (
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-lg">What was wrong with the splint?</FormLabel>
              <FormControl>
                <Textarea
      ...
      {formData.returnReason === "expensive" && (
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-lg">What would be an acceptable price for you?</FormLabel>
              <FormControl>
                <Input placeholder="e.g., $150, £120" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
      ...
      {formData.returnReason === "other" && (
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-lg">Please specify your reason for return</FormLabel>
              <FormControl>
                <Textarea
      ...
      <div className="form-group">
        ...
        <p className="mt-2 text-gray-600 text-sm">
          Photos help us understand your return better and process it faster.
        </p>
      </div>
    ...
    <div className="space-y-6">
      <div className="bg-gray-50 p-6 rounded-lg">
        ...
        <div className="gap-4 grid grid-cols-1 md:grid-cols-2 text-base">
          ...
          <div>
            ...
            {formData.country === "uk" ? "United Kingdom" : "United States"}
          </div>
          <div>
            <span className="font-medium">Reason:</span>{" "}
            {formData.returnReason === "uncomfortable" && "The splint was uncomfortable"}
            {formData.returnReason === "damaged" && "The splint was damaged"}
            {formData.returnReason === "expensive" && "The splint was too expensive"}
            {formData.returnReason === "other" && "Other"}
          </div>
        </div>

        {/* Additional Details Section */}
        {formData.returnReason === "uncomfortable" && (formData.comfortIssues?.length || 0) > 0 && (
          <div className="mt-4 pt-4 border-gray-200 border-t">
            ...
            {formData.otherComfortReason && (
              <div className="mt-2">
                <span className="font-medium">Other:</span> {formData.otherComfortReason}
              </div>
            )}
          </div>
        )}

        {formData.returnReason === "damaged" && formData.damageDescription && (
          <div className="mt-4 pt-4 border-gray-200 border-t">
            <span className="font-medium">Damage Description:</span>
            <p className="mt-2 text-gray-700 text-sm">{formData.damageDescription}</p>
          </div>
        )}

        {formData.returnReason === "expensive" && formData.acceptablePrice && (
          <div className="mt-4 pt-4 border-gray-200 border-t">
            <span className="font-medium">Acceptable Price:</span>
            <p className="mt-2 text-gray-700 text-sm">{formData.acceptablePrice}</p>
          </div>
        )}
        ...
        {formData.returnReason === "other" && formData.otherReason && (
          <div className="mt-4 pt-4 border-gray-200 border-t">
            ...
            <p className="mt-2 text-gray-700 text-sm">{formData.otherReason}</p>
          </div>
        )}
      </div>
      ...
      {submitError && (
        <Alert className="bg-red-50 border-red-200">
          ...
          <AlertDescription className="text-red-800 text-lg">{submitError}</AlertDescription>
        </Alert>
      )}
    ...
    <div className="space-y-6">
      <div className="bg-primary/10 p-6 border border-primary/30 rounded-lg">
        ...
        <p className="text-neutral-700">
          Thank you for your return request. We'll review it and get back to you within 24 hours.
        </p>
      </div>

      <div className="bg-blue-50 p-6 rounded-lg">
        <h3 className="mb-3 font-semibold text-lg">Return Instructions</h3>
        ...
        <p className="mt-3 text-gray-600 text-lg">
          💰 You will be refunded after receipt of the product. We aim to process refunds within 7
          days.
        </p>
      </div>
      ...
      <nav aria-label="Progress" className="mb-8">
        <ol className="flex justify-center items-center">
          {stages.map((stageItem, index) => (
            <li
              key={stageItem.key}
              className={`relative ${index !== stages.length - 1 ? "pr-8 sm:pr-20" : ""}`}
            >
              ...
              {index !== stages.length - 1 && (
                <div
                  className={`absolute top-4 left-8 -ml-px h-0.5 w-full z-0 ${
                    index < currentIndex ? "bg-primary/70" : "bg-gray-300"
                  }`}
                />
              )}
            </li>
        ...
        <CardContent className="p-8 w-full">
          <div className="space-y-8 w-full">
            <div>
              ...
              <p className="mt-2 text-gray-800 text-lg leading-relaxed">{getStageDescription()}</p>
            </div>
            ...
            {stage !== "complete" && getFAQContent() && (
              <div className="bg-blue-50 mt-8 p-4 border border-blue-200 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-blue-800 text-sm leading-relaxed">{getFAQContent()?.title}</p>
                </div>
              </div>
            )}
          </div>
