...
const PriceDisplay: React.FC<PriceDisplayProps> = ({
  ...
  if (variant === "badge") {
    return (
      ...
      >
        {formattedPrice}
        {showFreeShipping && <span className="text-primary/70 text-xs">+ Free Shipping</span>}
      </span>
    );
  }

  if (variant === "inline") {
    return <span className={cn("font-medium text-primary", className)}>{formattedPrice}</span>;
  }

  return (
    <div className={cn("flex flex-col", className)}>
      <div className="flex items-baseline gap-2">
        <span className={cn("font-bold text-neutral-900 dark:text-neutral-100", sizeClasses[size])}>
          {formattedPrice}
        </span>
      </div>
      {showFreeShipping && (
