...
import { Sheet, SheetContent, SheetFooter, SheetHeader, SheetTitle } from "@thetis/ui/sheet";
...
import { Check, Loader2, Minus, Plus, ShoppingBag, Sparkles, Video, X } from "lucide-react";
...
export function CartSheet() {
  ...
  const cartVariantIds = useMemo(() => cartLines.map((line) => line.merchandise.id), [cartLines]);

  const upsellSuggestions = useMemo(
    () =>
      getUpsellSuggestions(cartVariantIds).filter(
        (product) => product.variantId !== "gid://shopify/ProductVariant/52265315828040",
      ),
    [cartVariantIds],
  );
    ...
    if (cartLines.length > 0) {
      trackBeginCheckout(
        cartLines.map((line) => ({
          ...
          variant: line.merchandise.title !== "Default Title" ? line.merchandise.title : undefined,
        })),
      );
    }
          ...
          <>
                ...
                <div className="pt-4 border-neutral-200 border-t">
                  ...
                  <div className="space-y-3">
                    {upsellSuggestions.map((product) => (
                      <UpsellItem key={product.variantId} product={product} />
                    ))}
                  </div>
                </div>
              )}
            ...
            <SheetFooter className="pt-6 border-neutral-200 dark:border-neutral-700 border-t">
              <div className="space-y-4 w-full">
                ...
                <TrustClaims lang="en" variant="compact" align="center" />

                {/* Total */}
                <div className="flex justify-between items-baseline">
                  <span className="font-medium text-neutral-600 dark:text-neutral-400">Total</span>
...
function CartLineItem({ line, onUpdateQuantity, onRemove }: CartLineItemProps) {
  ...
  React.useEffect(() => {
    ...
    async function loadPrice() {
      ...
      try {
        ...
        const priceData = await getVariantPrice(line.merchandise.id, countryCode);
  ...
  const priceToUse = currentPrice || {
    amount: line.merchandise.price.amount,
    currencyCode: line.merchandise.price.currencyCode,
    formattedPrice: formatPrice(line.merchandise.price.amount, line.merchandise.price.currencyCode),
  };

  const lineTotal = (parseFloat(priceToUse.amount) * line.quantity).toFixed(2);

  return (
    <div className={`flex gap-4 p-3 bg-neutral-50 rounded-lg ${isUpdating ? "opacity-50" : ""}`}>
      {/* Product Image */}
      ...
      <div className="flex flex-col flex-1">
        <div className="flex justify-between items-start">
          <div>
            <h4 className="font-medium text-sm line-clamp-1">{line.merchandise.product.title}</h4>
            {variantTitle && <p className="text-neutral-500 text-xs">{variantTitle}</p>}
          </div>
          <button
        ...
        <div className="flex justify-between items-center mt-auto pt-2">
          ...
          <div className="flex items-center gap-2 bg-white border border-neutral-200 rounded-md">
            ...
            <span className="w-6 font-medium text-sm text-center">{line.quantity}</span>
            <button
              onClick={() => handleQuantityChange(line.quantity + 1)}
              disabled={isUpdating}
              className="hover:bg-neutral-100 disabled:opacity-50 p-1.5 transition-colors disabled:cursor-not-allowed"
          ...
          <span className="font-semibold">
            {isLoadingPrice ? (
              ...
              formatPrice(lineTotal, priceToUse.currencyCode)
            )}
          </span>
        </div>
      </div>
...
function UpsellItem({ product }: UpsellItemProps) {
  ...
  const courseType = isEssentials ? "standard" : isProfessionals ? "premium" : null;
    ...
    <div className="flex flex-col gap-2 bg-primary/5 hover:bg-primary/10 p-3 border border-primary/20 rounded-lg transition-colors">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-medium text-neutral-900 text-sm truncate">{product.title}</h4>
          <p className="text-neutral-500 text-xs truncate">{product.description}</p>
          ...
          <AccordionItem value="course-info" className="border-none">
            ...
            <AccordionContent className="pt-2 pb-0">
              <div className="space-y-4">
                ...
                {courseType === "premium" && "videoTopics" in course && course.videoTopics && (
                  <div>
