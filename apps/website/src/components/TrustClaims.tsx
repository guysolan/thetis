...
export function TrustClaims({
  ...
  return variant === "compact" ? (
    ...
    >
      ...
      >
        <span className="font-medium text-primary">✓ {t.freeShipping}</span>
        <span className="text-neutral-400">•</span>
        <span className="font-medium text-primary">✓ {t.shipsIn24Hours}</span>
        <span className="text-neutral-400">•</span>
        <span className="font-semibold text-primary">{t.patientsTrust}</span>
      </div>
    </div>
  ) : (
    <div
