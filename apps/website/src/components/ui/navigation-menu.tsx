...
const navigationMenuTriggerStyle = cva(
  "inline-flex justify-center items-center bg-background data-[active]:bg-accent/50 data-[state=open]:bg-accent/50 hover:bg-accent focus:bg-accent disabled:opacity-50 px-4 py-2 rounded-md w-max h-10 font-medium text-base text-neutral-900 hover:text-accent-foreground dark:hover:text-accent-foreground focus:text-accent-foreground dark:text-neutral-50 transition-colors cursor-pointer disabled:pointer-events-none focus:outline-none group",
);
...
>(({ className, ...props }, ref) => (
  <div className={cn("absolute left-0 top-full flex justify-start w-full")}>
    <NavigationMenuPrimitive.Viewport
      className={cn(
        "origin-top-left relative mt-2 h-[var(--radix-navigation-menu-viewport-height)] w-full overflow-hidden rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 text-popover-foreground shadow-xl data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-90 md:w-[var(--radix-navigation-menu-viewport-width)]",
        className,
      )}
      ref={ref}
      {...props}
