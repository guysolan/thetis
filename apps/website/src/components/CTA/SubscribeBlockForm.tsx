...
import { hasAnySubscription, markEmailAsSubscribed } from "@/lib/subscription-storage";
...
export function SubscribeBlockForm({
    ...
    return (
      <div
        className={cn(
          ...
          variant === "section" && "bg-primary/5 dark:bg-primary/10 py-8 md:py-10 px-6 md:px-8",
          variant === "compact" && "bg-primary/5 dark:bg-primary/10 p-5",
        )}
      >
        <div className="flex items-center gap-2 text-primary">
          <Mail className="w-4 h-4" />
          <p className="font-medium text-sm">{t.subscribed}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className={cn("flex flex-col gap-4", variant === "section" ? "max-w-xl" : "max-w-md")}>
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold text-neutral-900 dark:text-neutral-100">{t.title}</h3>
            <p className="text-neutral-600 dark:text-neutral-400 text-sm">{t.subtitle}</p>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="flex sm:flex-row flex-col gap-2 w-full">
          <input
            type="email"
            value={email}
          ...
          >
            {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : t.subscribe}
          </button>
        </form>
