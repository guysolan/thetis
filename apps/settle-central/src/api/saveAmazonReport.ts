...
const saveAmazonReport = async (region: string, report: AmazonReport) => {
  ...
  try {
    const { data, error } = await supabase.functions.invoke("save-amazon-report", {
      body: { report, region },
    });
...
export const useSaveAmazonReport = () => {
  ...
  return useMutation({
    mutationFn: ({ region, report }: { region: string; report: AmazonReport }) =>
