  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button type="button" onClick={(e) => e.stopPropagation()} variant="ghost" size="icon">
          <MoreVertical size={20} />
        </Button>
      </PopoverTrigger>
      <PopoverContent align="end" side="bottom" className="flex flex-col gap-1 p-1">
        {editForm && (
          <Sheet
            trigger={
        ...
        {onDuplicate && (
          <Button onClick={onDuplicate} className="justify-start gap-2 px-2" variant="ghost">
            <Copy size={20} />
            Duplicate
          </Button>
        )}
