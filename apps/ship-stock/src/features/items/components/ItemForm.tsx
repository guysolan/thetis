...
export const ItemForm = ({ item, itemType }: Props) => {
  ...
  return (
    <Form {...form}>
      <form id="item-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4 p-1">
        ...
        {visibility.countryOfOrigin && (
          <Input label="Country of Origin" name="country_of_origin" type="text" />
        )}

        {visibility.dimensions && (
          <div className="space-y-4">
            <h3 className="font-medium text-sm">Dimensions & Weight</h3>
            <div className="gap-4 grid grid-cols-2">
              <Input label="Height (cm)" name="height" type="number" />
              <Input label="Width (cm)" name="width" type="number" />
              <Input label="Depth (cm)" name="depth" type="number" />
