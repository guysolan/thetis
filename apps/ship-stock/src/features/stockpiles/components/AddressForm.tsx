...
interface Props {
  address:
    | (Address["Row"] & { companies: Company["Row"] })
    | null
    | {
        id: number;
        name: string | null;
...
export default function AddressForm({ address, operation, onSuccess }: Props) {
  ...
  const { mutate: companyAddressMutation } = useCompanyAddressMutation(operation);
  ...
  return (
    <Form {...form}>
      <form id="address-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4 p-1">
        <Select
          name="company_id"
          label="Company"
        ...
        <Input label="Address Line 2 (Optional)" name="line_2" type="text" />
        ...
        <div className="flex items-center space-x-2">
          <Checkbox
            ...
            onCheckedChange={(checked) => form.setValue("holds_stock", checked as boolean)}
          />
          <label
            htmlFor="holds_stock"
        ...
        <Button form="address-form" type="button" onClick={() => form.handleSubmit(onSubmit)()}>
          Save Changes
        </Button>
      </form>
    </Form>
