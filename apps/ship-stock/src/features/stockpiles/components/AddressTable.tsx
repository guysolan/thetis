      ...
      <TableBody>
        {addresses?.map((address) => (
          <TableRow key={address.id}>
            ...
            <TableCell className="min-w-32">{address.line_1}</TableCell>
            <TableCell>{address.line_2}</TableCell>
            <TableCell>{address.city}</TableCell>
            <TableCell>{address.region}</TableCell>
            <TableCell className="min-w-16">{address.holds_stock ? "Yes" : "No"}</TableCell>
            ...
            <TableCell>
              <ActionPopover
                title="Address"
                editForm={
                  <AddressForm address={address} operation="upsert" companyId={companyId} />
                }
                deleteFunction={() => deleteAddress(address.id)}
              />
            </TableCell>
          </TableRow>
        ))}
