...
import { getCurrentQuantity, getOrderTypeBadgeVariant } from "../../stock-history/utils";
...
export default function ItemHistorySheet({
  ...
  const transactionRows = useMemo<TransactionRow[]>(() => {
    ...
    for (const record of inventoryHistory) {
      ...
      const matchingItems = record.items.filter((item) => item.id === itemId);
      ...
      for (const item of matchingItems) {
        ...
        const col = stockpileColumns.find((c) => c.addressId === item.address_id);
        if (col) {
          locations[col.key] = {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="bg-neutral-50 sm:max-w-2xl">
        <SheetHeader>
          ...
          <SheetDescription>Stock history across all locations</SheetDescription>
        </SheetHeader>
                      ...
                      {stockpileColumns.map((col) => {
                        ...
                        return (
                          <TableCell key={col.key}>
                            <div>
                              <div className="tabular-nums">{loc.quantity}</div>
                              {loc.change !== 0 && (
                                <div
                                  className={`text-xs ${
                                    loc.change > 0 ? "text-green-600" : "text-red-600"
                                  }`}
                                >
                                  {loc.change > 0 ? "+" : ""}
                                  {loc.change}
