...
export const useCompanyAutoFill = (
  ...
  const companyId = form.watch(direction === "to" ? "to_company_id" : "from_company_id");
  useEffect(() => {
    if (!companyId) return;
    ...
    const hasExistingValues = currentBilling || currentShipping || currentContact;
    ...
    if (!hasExistingValues) {
      ...
      if (defaultShipping?.id) {
        form.setValue(shipping, String(defaultShipping.id));
      } else if (addresses.length === 1) {
        form.setValue(shipping, String(addresses[0].id));
      }
      // Set default billing address
      const defaultBilling = addresses.find((a) => a.is_default_billing);
      if (defaultBilling) {
        form.setValue(billing, String(defaultBilling.id));
      } else if (addresses.length === 1) {
        form.setValue(billing, String(addresses[0].id));
      }

      // Set default contact
      const defaultContact = contacts.find((c) => c.is_default);
      if (defaultContact) {
        form.setValue(contact, String(defaultContact.id));
      } else if (contacts.length === 1) {
        form.setValue(contact, String(contacts[0].id));
      }
    }
  }, [companyId]);
};
