...
export const useCompanyForm = (direction: "to" | "from") => {
  ...
  const getFieldName = (type: "company" | "shipping" | "billing" | "contact") => {
  ...
  const getSelectedAddress = (type: "shipping" | "billing") => {
    ...
    return addresses?.find((addr) => String(addr.id) === addressId);
  };
  ...
  const getSelectedContact = () => {
    ...
    return contacts?.find((cont) => String(cont.id) === contactId)?.name;
  };
