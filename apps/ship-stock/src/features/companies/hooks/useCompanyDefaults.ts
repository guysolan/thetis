...
const useCompanyDefaults = ({ fieldName }: { fieldName?: "to_company_id" | "from_company_id" }) => {
  ...
  React.useEffect(() => {
    if (companyUser?.company_id) {
      form.setValue("company_id", companyUser.company_id.toString());
      console.log(`Setting ${fieldName} to ${companyUser.company_id}`);
      if (fieldName) {
        form.setValue(fieldName, companyUser.company_id.toString());
      }
    }
