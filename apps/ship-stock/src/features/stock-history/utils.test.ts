...
describe("Stock History Utils", () => {
  ...
  it("creates current stock record correctly", () => {
    ...
    const result = createCurrentStockRecord(mockInventoryHistory, visibleItems);
  ...
  it("creates current stock record correctly with address filter", () => {
    ...
    const result = createCurrentStockRecord(mockInventoryHistory, visibleItems, 1);
  ...
  it("gets item quantity at time correctly with address filter", () => {
    ...
    const result1 = getItemQuantityAtTime(1, mockInventoryHistory, date1, 1);
    expect(result1).toBe(150);

    const result2 = getItemQuantityAtTime(2, mockInventoryHistory, date1, 1);
    expect(result2).toBe(0);
  });
