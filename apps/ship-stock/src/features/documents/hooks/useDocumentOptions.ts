...
export const useDocumentOptions = (documentType: DocumentType) => {
  ...
  const documentOptions: DocumentOptions = {
    shippingDetails: {
      show: search?.shippingDetails?.show ?? defaults.shippingDetails.show,
      reasonForExport:
        search?.shippingDetails?.reasonForExport ?? defaults.shippingDetails.reasonForExport,
      modeOfTransport:
        search?.shippingDetails?.modeOfTransport ?? defaults.shippingDetails.modeOfTransport,
      incoterms: search?.shippingDetails?.incoterms ?? defaults.shippingDetails.incoterms,
      unitOfMeasurement:
        search?.shippingDetails?.unitOfMeasurement ?? defaults.shippingDetails.unitOfMeasurement,
      shipmentNumber:
        search?.shippingDetails?.shipmentNumber ?? defaults.shippingDetails.shipmentNumber,
      airwaybill: search?.shippingDetails?.airwaybill ?? defaults.shippingDetails.airwaybill,
      referenceNumber:
        search?.shippingDetails?.referenceNumber ?? defaults.shippingDetails.referenceNumber,
    },
    from: {
      ...
      paymentMethods: Object.fromEntries(
        Object.keys(PAYMENT_METHODS).map((method) => [
          ...
          search?.payment?.paymentMethods?.[method] ??
            defaults.payment.paymentMethods?.[method] ??
            true,
        ]),
      ),
    },
    ...
    showShippingItems: search?.showShippingItems ?? defaults.showShippingItems ?? true,
    showItemsManifest: (search as any)?.showItemsManifest ?? defaults.showItemsManifest ?? true,
  ...
  return {
    ...documentOptions,
    showFinancials: (search as any)?.showFinancials ?? (defaults as any).showFinancials ?? true,
  } as DocumentOptions;
};
