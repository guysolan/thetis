...
export async function saveOrderCompanies(
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      from_billing_address_id: toNullIfEmpty(data.from_billing_address_id),
      from_shipping_address_id: toNullIfEmpty(data.from_shipping_address_id),
 ...
 */
export async function saveOrderItems(orderId: number, formData: MultiOrderFormData): Promise<void> {
  ...
  if (existingOrderItemChanges && existingOrderItemChanges.length > 0) {
    const itemChangeIds = existingOrderItemChanges.map((oic) => oic.item_change_id);
    ...
    if (deleteOrderItemChangesError) {
      console.error("❌ Error deleting order_item_changes:", deleteOrderItemChangesError);
    ...
    if (itemChangeIds.length > 0) {
      ...
      if (deleteItemChangesError) {
        console.error("❌ Error deleting item_changes:", deleteItemChangesError);
  ...
  const packageItems = (formData.package_items || []).filter((p): p is typeof p => {
    const hasPackageId = p.package_id != null && String(p.package_id).trim() !== "";
    if (!hasPackageId) {
      console.warn("⚠️ Skipping package item with missing package_id:", p);
      return false;
    }
    return true;
  });
  ...
  const packageAddressIdNum = packageAddressId != null ? Number(packageAddressId) : NaN;
  if (packageItems.length > 0 && !Number.isNaN(packageAddressIdNum)) {
    const packageItemPromises = packageItems.map(async (packageItem) => {
      ...
      if (Number.isNaN(packageIdNum)) {
        throw new Error(`Invalid package_id for package item: ${packageItem.package_id}`);
      }
      ...
      if (itemChangeError) {
        console.error("❌ Error upserting package item_change:", itemChangeError);
      ...
      if (orderItemChangeError) {
        console.error("❌ Error upserting package order_item_change:", orderItemChangeError);
  ...
  if (regularItems.length > 0) {
    const regularItemPromises = regularItems.map(async (item) => {
      ...
      if (itemChangeError) {
        console.error("❌ Error upserting regular item_change:", itemChangeError);
        throw itemChangeError;
      }
      ...
      if (orderItemChangeError) {
        console.error("❌ Error upserting regular order_item_change:", orderItemChangeError);
        throw orderItemChangeError;
      }
    });
  ...
  const quotePayload =
    formData.order_type === "quote"
      ? (() => {
          const priceBands: Record<string, number> = {};
          for (const qty of PRICE_BAND_QUANTITIES) {
            const priceVal = (formData as Record<string, unknown>)[`quote_price_${qty}`];
            const price = typeof priceVal === "string" ? parseFloat(priceVal) : Number(priceVal);
            if (!Number.isNaN(price) && price > 0) {
              const qtyVal = (formData as Record<string, unknown>)[`quote_quantity_${qty}`];
  ...
  if (updateFormValuesError) {
    console.error("❌ Error updating order_form_values:", updateFormValuesError);
