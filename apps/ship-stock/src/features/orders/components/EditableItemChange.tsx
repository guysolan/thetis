...
export const EditableItemChange = ({
  ...
  const onSubmit = async (data: ItemChangeFormData) => {
    ...
    const transformedData = {
      ...data,
      address_id:
        data.address_id === "" || data.address_id === null || data.address_id === undefined
          ? null
          : Number(data.address_id),
      item_id:
        data.item_id === "" || data.item_id === null || data.item_id === undefined
          ? undefined
          : Number(data.item_id),
    };
  ...
  return (
    <Card className="mb-4">
      <CardHeader>
        <CardTitle className="text-sm">Item Change: {itemChange.items.name}</CardTitle>
      ...
      <CardContent>
        <FormProvider {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button type="submit" size="sm" disabled={updateItemChange.isPending}>
