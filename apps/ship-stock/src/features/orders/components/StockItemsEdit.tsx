      ...
      <TableBody>
        {fields.map((field, index) => {
          ...
          const item = items?.find(
            (i) => String(i.item_id) === form.watch(`${name}.${index}.item_id`),
          );
          return (
            <TableRow key={field.id}>
              {allowedTypes.length > 1 && (
                <TableCell>{form.watch(`${name}.${index}.item_type`)}</TableCell>
              )}
              <TableCell>{item?.item_name || "-"}</TableCell>
              <TableCell>{form.watch(`${name}.${index}.quantity_change`)}</TableCell>
              <TableCell>
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row justify-between items-center space-y-0 pb-2">
        <CardTitle className="font-medium text-base">Stock Consumed at {address?.name}</CardTitle>
        <Button
          type="button"
          variant="ghost"
          size="icon"
                        ...
                        <Select
                          ...
                          options={
                            items
                              ?.filter(
                                (item) =>
                                  item.item_type === form.watch(`${name}.${index}.item_type`),
                              )
                              .map((item) => ({
