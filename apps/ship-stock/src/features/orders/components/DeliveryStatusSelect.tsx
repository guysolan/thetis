...
import { AlertTriangle, CheckCircle, Clock, Package, PackageCheck, Truck } from "lucide-react";
...
export function DeliveryStatusSelect({ orderId, currentStatus }: DeliveryStatusSelectProps) {
  ...
  const statusKey = (currentStatus in statusConfig ? currentStatus : "pending") as DeliveryStatus;
  const statusColor = statusConfig[statusKey]?.color || statusConfig.pending.color;
  const StatusIcon = statusConfig[statusKey]?.icon || statusConfig.pending.icon;
