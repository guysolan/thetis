...
interface PriceItemsFormFieldsProps {
  ...
  getFilteredItemOptions: (itemType: string) => { label: string; value: string }[];
      ...
      <TableBody>
          ...
          <TableRow key={field.id}>
            ...
            <TableCell>
              <Select
                ...
                options={getFilteredItemOptions(form.watch(`${name}.${index}.item_type`))}
              />
            </TableCell>
            <TableCell>
              <Input name={`${name}.${index}.quantity_change`} type="number" />
            </TableCell>
            {showPrice && (
              <TableCell>
                <Input name={`${name}.${index}.item_price`} type="number" step="0.01" />
              </TableCell>
            )}
            {showPrice && (
              <TableCell>
                <Input name={`${name}.${index}.item_tax`} type="number" step="0.01" />
              </TableCell>
            )}
        ...
        {showPrice && fields.length > 0 && (
          <TableRow className="font-semibold">
            ...
            <TableCell>
              <div className="flex gap-2">
                <Button type="button" variant="secondary" className="px-2">
                  <Copy size={20} />
                </Button>
                <Button type="button" variant="destructive" className="px-2">
                  <Trash size={20} />
                </Button>
              </div>
            </TableCell>
