...
export const useStockQuantities = (
  ...
  const { data: inventoryHistory } = useQuery(selectInventoryHistoryQueryOptions());
  ...
  const getItemQuantities = (itemId: string) => {
    const quantityChange =
      itemChanges?.find((i) => String(i.item_id) === String(itemId))?.quantity_change ?? 0;
    ...
    if (orderDate && inventoryHistory && inventoryHistory.length > 0) {
      const dateBeforeOrder = dayjs(orderDate).subtract(1, "day").format("YYYY-MM-DD");
