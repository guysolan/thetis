...
export const useStocktakeQuantities = (name: string, address_name: string) => {
  ...
  const getItemQuantities = (itemId: string) => {
    ...
    const stockpileItem = stockpileItems.find(
      (w) =>
        String(w.address_id) === String(selectedAddress) && String(w.item_id) === String(itemId),
    );

    const currentQuantity = Number(stockpileItem?.item_quantity ?? 0);
    const itemChange = itemChanges?.find((i) => String(i.item_id) === String(itemId));
    const quantityAfter = Number(itemChange?.quantity_after ?? currentQuantity);
  ...
  const updateItemQuantities = (itemId: string) => {
    ...
    const stockpileItem = stockpileItems?.find((i) => String(i.item_id) === String(itemId));
    const currentQuantity = Number(stockpileItem?.item_quantity ?? 0);
    ...
    const fieldIndex = fields.findIndex((field) => String(field.item_id) === String(itemId));

    if (fieldIndex === -1) return;

    const quantityAfter = Number(
      form.watch(`${name}.${fieldIndex}.quantity_after`) ?? currentQuantity,
    );
    const quantityChange = quantityAfter - currentQuantity;
