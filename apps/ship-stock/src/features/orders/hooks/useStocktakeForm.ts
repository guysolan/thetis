...
export const useStocktakeForm = (name: string, address_name: string) => {
  ...
  const itemsValues = useWatch({
    ...
    name:
      items
        ?.map((_, index) => [`${name}.${index}.item_id`, `${name}.${index}.quantity_after`])
        .flat() ?? [],
  });
  ...
  useEffect(() => {
    ...
    items.forEach((item, index) => {
      ...
      const currentQuantityBefore = form.getValues(`${name}.${index}.quantity_before`);
      const currentQuantityAfter = form.getValues(`${name}.${index}.quantity_after`);
      ...
      if (
        ...
        form.setValue(`${name}.${index}.quantity_after`, currentQuantity);
      }

      // Recalculate quantity_change
      const quantityAfter = form.getValues(`${name}.${index}.quantity_after`);
      const quantityChange = Number(quantityAfter) - currentQuantity;
      form.setValue(`${name}.${index}.quantity_change`, quantityChange);
    });
  }, [items, itemsValues, inventoryHistory, selectedAddress, name, form.setValue]);
