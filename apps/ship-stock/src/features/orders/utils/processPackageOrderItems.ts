...
import { DisplayItem, OrderItemChange, ProcessedItems, ProcessOrderItemsParams } from "../types";
...
const processPackageComponents = (
  ...
  for (const component of item.components) {
    ...
    const componentQuantity = component.component_quantity * Number(orderItem.quantity_change);
...
export const processPackageOrderItems = ({
  ...
  orderItems.forEach((orderItem) => {
    ...
    const item = items?.find((w) => String(w.item_id) === String(orderItem.item_id));
    ...
    if (item.item_type === "package") {
      ...
      const { fromItems: packageFromItems, toItems: packageToItems } = processPackageComponents(
        item,
        orderItem,
        fromShippingAddressId,
        toShippingAddressId,
      ...
      processedComponents.set(
        ...
        (processedComponents.get(String(orderItem.item_id)) || 0) + quantityChange,
      );
    }
  });
  ...
  processedComponents.forEach((totalQuantity, itemId) => {
    ...
    const fromItem = fromItems.find((item) => String(item.item_id) === itemId);
    if (fromItem) {
