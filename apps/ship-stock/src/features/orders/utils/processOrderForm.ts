...
export const calculateRequiredQuantity = (
  ...
  return parseFloat(String(componentQuantity)) * parseFloat(String(producedQuantity));
};
...
export const findStockLevel = (
  ...
  const stockItem = addressStock.find((item) => String(item.item_id) === String(componentId));
  ...
  const producedQuantity = producedItems.reduce((sum, producedItem) => {
    ...
    const product = items?.find((p) => String(p.item_id) === String(producedItem.item_id));
  ...
  return (stockItem ? parseFloat(String(stockItem.item_quantity)) : 0) + producedQuantity;
};

export const createServiceItem = (component: Component, requiredQuantity: number): OrderItem => ({
...
export const processComponent = (
  ...
  if (component.component_type === "service") {
    const serviceItem = createServiceItem(
      component,
      calculateRequiredQuantity(component.component_quantity, producedQuantity),
    );
    return {
      consumedItems: [],
  ...
  const currentStock = findStockLevel(component.component_id, addressStock, producedItems);
  ...
  const result = {
    consumedItems: currentStock > 0 ? [createConsumedItem(component, currentStock)] : [],
    purchaseItems: [createPurchaseItem(component, requiredQuantity - currentStock)],
  };
