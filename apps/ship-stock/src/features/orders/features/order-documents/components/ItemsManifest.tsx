...
const ItemsManifest = ({ orderItems }: { orderItems: OrderView["items"] }) => {
  ...
  const contentItems = Object.values(groupedItems).filter((item) => item.totalQuantity > 0);
        ...
        <TableBody>
          {contentItems.map((item) => (
            <TableRow key={item.item_id}>
              ...
              <TableCell className="text-black">{item.lot_number}</TableCell>
              <TableCell className="text-black text-right">{item.totalQuantity}</TableCell>
            </TableRow>
          ))}
          <TableRow className="font-semibold">
            <TableCell className="text-black">Total Items</TableCell>
            <TableCell colSpan={4} />
            <TableCell className="text-black text-right">
              {contentItems.reduce((sum, item) => sum + item.totalQuantity, 0)}
            </TableCell>
          </TableRow>
        </TableBody>
      </Table>
