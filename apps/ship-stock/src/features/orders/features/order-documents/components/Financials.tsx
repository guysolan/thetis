...
const Financials = ({ order, currency }: { order: OrderView; currency: Currency }) => {
  const { data: items } = useSelectItemsView();

  // Filter and group product items (items with pricing and cost > 0)
  const filteredItems = order.items.filter(
    (item) => item.quantity > 0 && item.item_type !== "service" && (item.price ?? 0) > 0,
  );
  ...
  const productItems = Object.values(groupedItems).filter((item) => item.totalQuantity > 0);
  ...
  const combinedServices = serviceItems.reduce<CombinedService[]>((acc, item) => {
    const existingService = acc.find((service) => service.item_id === Number(item.item_id));
  ...
  // Calculate totals
  const itemTotal = productItems.reduce((sum, item) => sum + item.totalPrice, 0);
  const servicesTotal = combinedServices.reduce((sum, service) => sum + service.total, 0);
        ...
        <TableBody>
          ...
          {combinedServices.map((service) => (
            <TableRow key={service.item_id}>
              <TableCell className="text-black">{service.item_name}</TableCell>
              <TableCell className="text-black text-right">{service.quantity}</TableCell>
              <TableCell className="text-black text-right">
