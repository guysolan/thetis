...
export const selectOrderFormValuesById = async (id: string) => {
  ...
  const validPackageIds = new Set(
    (data.order_form_values?.package_items || []).map((pkg) => pkg.package_item_change_id),
  );

  const cleanFromItems = (data.order_form_values?.from_items || []).filter(
    (item) => !item.package_item_change_id || validPackageIds.has(item.package_item_change_id),
  );

  const cleanToItems = (data.order_form_values?.to_items || []).filter(
    (item) => !item.package_item_change_id || validPackageIds.has(item.package_item_change_id),
  );
  ...
  const mergedFormValues = {
    ...
    from_company_id: data.from_company_id?.toString() || data.order_form_values?.from_company_id,
    from_billing_address_id:
      data.from_billing_address_id?.toString() || data.order_form_values?.from_billing_address_id,
    from_shipping_address_id:
      data.from_shipping_address_id?.toString() || data.order_form_values?.from_shipping_address_id,
    to_company_id: data.to_company_id?.toString() || data.order_form_values?.to_company_id,
    to_billing_address_id:
      data.to_billing_address_id?.toString() || data.order_form_values?.to_billing_address_id,
    to_shipping_address_id:
      data.to_shipping_address_id?.toString() || data.order_form_values?.to_shipping_address_id,
    from_contact_id: data.from_contact_id?.toString() || data.order_form_values?.from_contact_id,
    to_contact_id: data.to_contact_id?.toString() || data.order_form_values?.to_contact_id,
    ...
    reason_for_export: data.reason_for_export || data.order_form_values?.reason_for_export,
    shipment_number: data.shipment_number || data.order_form_values?.shipment_number,
    airwaybill: data.airwaybill || data.order_form_values?.airwaybill,
    mode_of_transport: data.mode_of_transport || data.order_form_values?.mode_of_transport,
    incoterms: data.incoterms || data.order_form_values?.incoterms,
    unit_of_measurement: data.unit_of_measurement || data.order_form_values?.unit_of_measurement,
    delivery_dates: data.delivery_dates || data.order_form_values?.delivery_dates,
  };
