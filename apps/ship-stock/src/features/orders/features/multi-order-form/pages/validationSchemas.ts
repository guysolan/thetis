...
export const orderDetailsSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.order_type !== "count" && data.order_type !== "quote") {
      if (
        !data.delivery_dates ||
        !Array.isArray(data.delivery_dates) ||
...
export const companiesAddressesSchema = z.object({
  ...
  from_billing_address_id: z.string().min(1, "From billing address is required"),
  from_shipping_address_id: z.string().min(1, "From shipping address is required"),
  to_billing_address_id: z.string().min(1, "To billing address is required"),
  to_shipping_address_id: z.string().min(1, "To shipping address is required"),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.order_type === "quote") {
      const hasAnyPrice = PRICE_BAND_QUANTITIES.some((q) => {
        const val = (data as Record<string, unknown>)[`quote_price_${q}`];
