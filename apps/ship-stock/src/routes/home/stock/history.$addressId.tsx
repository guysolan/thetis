...
const fetchInventoryHistory = async (addressId: string) => {
  ...
  return data.map((record) => ({
    ...record,
    order_type: record.order_type as "build" | "buy" | "sell" | "ship" | "count",
...
const fetchLocations = async () => {
  const { data, error } = await supabase
    .from("addresses")
    .select("id, name, line_1, line_2, city, region, code, country, holds_stock")
    .eq("is_active", true)
    .eq("holds_stock", true)
    .order("name");
...
function StockHistoryPage() {
  ...
  const currentLocation = locations.find((location) => location.id.toString() === addressId);
    ...
    <div className="space-y-6 mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-xl">{currentLocation?.name ?? "History"}</h1>
        </div>
        ...
        <Select value={addressId} onValueChange={handleLocationChange}>
          ...
          <SelectContent>
            {locations.map((location) => (
              <SelectItem key={location.id} value={location.id.toString()} className="py-2">
                <div>
                  <div className="font-medium">{location.name}</div>
                  {(location.line_1 || location.city) && (
                    <div className="mt-0.5 text-muted-foreground text-xs truncate">
                      {[location.line_1, location.city, location.region].filter(Boolean).join(", ")}
                    </div>
                  )}
                </div>
              </SelectItem>
      ...
      {!inventoryHistory || inventoryHistory.length === 0 ? (
        ...
        <StockHistoryTable inventoryHistory={inventoryHistory} addressId={Number(addressId)} />
      )}
    </div>
  );
}
...
export const Route = createFileRoute("/home/stock/history/$addressId")({
  ...
  loader: async ({ params, context }) => {
    ...
    const [inventoryHistory, locations] = await Promise.all([
      context.queryClient.ensureQueryData(getInventoryHistoryQueryOptions(addressId)),
