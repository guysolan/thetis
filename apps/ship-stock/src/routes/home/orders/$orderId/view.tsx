...
import { OrderWithDetails, useSelectOrderById } from "@/features/orders/api/selectOrderById";
...
export const Route = createFileRoute("/home/orders/$orderId/view")({
...
const createFlows = (orderItemChanges: OrderWithDetails["order_item_changes"]): FlowItem[] => {
...
const renderFlowBadge = (flow: FlowItem, currency: string, itemId: string, idx: number) => {
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      <span className="font-medium text-muted-foreground">{flow.from}</span>
...
function RouteComponent() {
  ...
  const { data: order, isLoading: orderLoading } = useSelectOrderById(orderId);
  const { data: addresses, isLoading: addressesLoading } = useSelectAddresses();
    ...
    const itemChangeOptions = order.order_item_changes
      .map((orderItemChange) => ({
        ...
        label: `${orderItemChange.item_changes.items.name} (${
          ...
          orderItemChange.item_changes.addresses?.name || "Unknown Address"
        })`,
  ...
  const groupedOrderItems = React.useMemo(() => {
    ...
    return order.order_item_changes.reduce(
      (acc, orderItemChange) => {
      ...
      <Card>
        ...
        <CardContent>
          <Accordion type="multiple" className="space-y-4 w-full">
            {Object.entries(groupedOrderItems).map(([itemId, { item, orderItemChanges }]) => {
              const flows = createFlows(orderItemChanges);

              return (
                <AccordionItem key={itemId} value={itemId} className="">
                  <AccordionTrigger className="px-4 py-4">
                    <div className="flex justify-between items-center mr-4 w-full">
                      <div className="flex items-center gap-2">
                        ...
                        {item.sku && (
                          <span className="text-muted-foreground text-sm">({item.sku})</span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        {flows.map((flow, idx) => (
                          ...
                          >
                            {renderFlowBadge(flow, order.currency || "£", itemId, idx)}
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <AccordionContent className="px-4 pb-4">
                    <div className="space-y-6">
                        ...
                        <CardContent>
                          <div className="gap-4 grid grid-cols-2 md:grid-cols-4 text-sm">
                            ...
                            <div>
                              ...
                              <p className="text-muted-foreground">{item.price || "N/A"}</p>
                            </div>
                            <div>
                              <p className="font-medium">HS Code</p>
                              <p className="text-muted-foreground">{item.hs_code || "N/A"}</p>
                            </div>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      {orderItemChanges.map((orderItemChange, changeIndex) => (
                        <div
                          key={`${orderItemChange.order_id}-${orderItemChange.item_change_id}`}
                          className="space-y-4"
                        >
                          <div className="flex items-center gap-2">
                            <h4 className="font-medium text-md">Change #{changeIndex + 1}</h4>
                            <Badge variant="outline" className="text-xs">
                              Qty: {orderItemChange.item_changes.quantity_change}
                            </Badge>
                          </div>
                          ...
                          <div className="gap-6 grid md:grid-cols-2">
                            {/* Order Item Change */}
                            <div>
                              <h5 className="mb-3 font-medium text-sm">Order Item Details</h5>
                            ...
                            {/* Item Change */}
                            <div>
                              <h5 className="mb-3 font-medium text-sm">Item Change Details</h5>
                              <EditableItemChange
                                orderItemChange={orderItemChange}
                                orderId={orderId}
                          ...
                          {changeIndex < orderItemChanges.length - 1 && (
