...
const StockCheckDialog: React.FC<StockCheckDialogProps> = ({ addressId, stockpileName }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Stock Check - {stockpileName}</DialogTitle>
        </DialogHeader>
        <div className="py-4">
          <StockCheckForm addressId={addressId} onSuccess={handleSuccess} />
        </div>
      </DialogContent>
    </Dialog>
  );
