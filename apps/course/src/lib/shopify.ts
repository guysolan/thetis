...
export async function getProductIdFromHandle(handle: string): Promise<string | null> {
  const query = `
  ...
  try {
    const response = await fetch(`https://shop.thetismedical.com/api/2024-01/graphql.json`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Shopify-Storefront-Access-Token": "784883c28d6484a8804b44ae00adfb99",
      },
      body: JSON.stringify({
        query,
        variables: { handle },
