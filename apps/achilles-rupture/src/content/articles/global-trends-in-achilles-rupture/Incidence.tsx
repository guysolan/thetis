...
const PlaceholderChart = () => (
  <div className="flex flex-col justify-center items-center bg-gray-50 border border-gray-200 rounded h-[300px]">
...
// Custom tooltip component with source information
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    const yearData = payload[0].payload;

    return (
      <div className="flex flex-col gap-y-1 bg-white shadow-md p-2 border border-gray-200 rounded max-w-xs">
        <h4 className="mb-1 font-bold text-xs">{label}</h4>
  ...
  <div className="mt-3">
    <Accordion type="single" collapsible className="w-full">
      <AccordionItem value="sources">
        ...
        <AccordionContent>
          <ol className="space-y-1.5 pl-5 text-xs list-decimal">
  ...
  return (
    <div className="space-y-3 w-full">
      <h3 className="font-medium text-xl text-center">
        Rising Incidence of Achilles Tendon Ruptures (1994-2021)
      </h3>
      <p className="text-sm text-center">
        Average incidence rates per 100,000 people and estimated global cases
        (millions)
      </p>
      ...
      {hasError || chartData.length === 0 ? (
        ...
        <div className="w-full h-[500px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart
              data={chartData}
              margin={{ top: 35, right: 75, left: 35, bottom: 30 }}
            >
      ...
      <p className="text-gray-600 text-xs text-center">
        Data from national health registries and research studies. Years without
        data points use interpolated values. Global estimates based on average
        incidence applied to estimated world population.
      </p>
