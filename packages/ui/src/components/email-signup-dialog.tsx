...
export interface EmailSignupDialogProps {
  ...
  onSubmit?: (data: { email: string; phone?: string; ruptureDate: Date }) => void | Promise<void>;
...
function EmailSignupDialog({
  ...
  description = "Receive personalized recovery guidance timed to your injury date. We'll send you the right information exactly when you need it.",
  ...
  const [ruptureDate, setRuptureDate] = React.useState<Date | undefined>(undefined);
  ...
  const setOpen = isControlled ? (newOpen: boolean) => onOpenChange?.(newOpen) : setInternalOpen;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (onSubmit) {
        ...
        throw new Error("Either onSubmit callback or supabaseClient must be provided");
      }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          {showPhone && (
            <div className="space-y-2">
              <Label htmlFor="email-signup-phone">{phoneLabel}</Label>
              <Input
            ...
            <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {ruptureDate ? format(ruptureDate, "PPP") : <span>{datePlaceholder}</span>}
                </Button>
              </PopoverTrigger>
