...
function stripTypeAnnotation(ts: string): string {
  ...
  let js = ts.replace(/export const patients: Review\[\] = /, "const patients = ");
  // Remove all import and export type lines
  js = js
    .split("\n")
    .filter((line) => !line.trim().startsWith("import") && !line.trim().startsWith("export type"))
    .join("\n");
  return js;
}
...
function extractReviewsFromAST(ast: any): any[] {
  ...
  for (const node of ast.body) {
    if (node.type === "VariableDeclaration" && node.declarations[0].id.name === "patients") {
            ...
            for (const prop of el.properties) {
              if (prop.type === "Property") {
                ...
                if (value.type === "Literal") {
                  ...
                  obj[key] = value.quasis.map((q: any) => q.value.cooked).join("");
                } else if (value.type === "ArrayExpression") {
                  obj[key] = value.elements.map((e: any) => e.value);
                } else if (value.type === "ObjectExpression") {
                  obj[key] = {};
                  for (const subprop of value.properties) {
                    if (subprop.type === "Property") {
                      const subkey = subprop.key.name || subprop.key.value;
                      obj[key][subkey] = subprop.value.value;
                    }
                  }
