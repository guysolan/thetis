-- Expose order delivery window on inventory history for UI (first = lower, second = upper of tstzrange).
...
WITH item_changes_with_dates AS (
  SELECT
    ...
    (
      ...
      FROM companies
      WHERE companies.id = o_1.from_company_id
    ) AS from_company_name,
    (
      SELECT companies.name
      FROM companies
      WHERE companies.id = o_1.to_company_id
    ) AS to_company_name
