-- Add new order_type enum values: sell, ship, count (replacing sale, shipment, stocktake)
ALTER TYPE order_type ADD VALUE IF NOT EXISTS 'sell';
ALTER TYPE order_type ADD VALUE IF NOT EXISTS 'ship';
ALTER TYPE order_type ADD VALUE IF NOT EXISTS 'count';

-- Migrate existing data to new values
UPDATE orders SET order_type = 'sell'::order_type WHERE order_type = 'sale';
UPDATE orders SET order_type = 'ship'::order_type WHERE order_type = 'shipment';
UPDATE orders SET order_type = 'count'::order_type WHERE order_type = 'stocktake';

-- Recreate orders_view to use 'sell' instead of 'sale' for total calculations
...
SELECT
    ...
    sum(
        CASE WHEN o.order_type = 'sell'::order_type THEN
    ...
    sum(
        CASE WHEN ic.quantity_change > 0 THEN
            oic.price * ic.quantity_change *(1::numeric + COALESCE(oic.tax, 0::numeric))
        ELSE
            0::numeric
