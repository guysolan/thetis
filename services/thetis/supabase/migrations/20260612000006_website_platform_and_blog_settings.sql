-- Add website as a platform option for saved settings.
ALTER TABLE public.assistant_instruction_sets
  DROP CONSTRAINT IF EXISTS assistant_instruction_sets_platforms_check;

ALTER TABLE public.assistant_instruction_sets
  ADD CONSTRAINT assistant_instruction_sets_platforms_check CHECK (platforms <@ ARRAY['linkedin', 'instagram', 'facebook', 'website']::text[]);

-- Example saved settings: website blog visual guides from uploaded images.
...
VALUES
  (
    'Recovery Architect — Tintin Edition (Website Blog)',
    ARRAY['website']::text[],
    'patient_education',
    'both',
...
[Website Blog: The Visual Guide Article]

Title: A clear, patient-friendly headline (under 70 characters).
Meta description: One sentence summarising the article for search (under 155 characters).
Opening hook: A relatable recovery milestone or common mistake — draw the reader in within two sentences.
Body: 4–6 short sections with descriptive subheadings. Follow the Clear Path (Problem → Insight → Action). Explain the why behind the science in plain language. Use bolding for the most important clinical takeaway in each section.
Illustration placements: Note where Tintin-style graphics should sit in the article (e.g., "After section 2").
Closing: A supportive summary and, where relevant, when to seek medical help.
Length: 600–900 words unless the user asks otherwise.
