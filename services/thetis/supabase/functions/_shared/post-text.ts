...
function formatSlideObject(
  ...
  // Mini-paragraph format: ALL CAPS headline + short body (no labels)
...
export function coercePostText(value: unknown): string | undefined {
  ...
  if (typeof value === "object") {
    ...
    if (Array.isArray(obj.slides)) {
      const slides = coercePostText(obj.slides);
...
export function normalizeAssistResult(
  ...
  const post = coercePostText(result.post) ??
    coercePostText({
      slides: result.slides,
      caption: result.caption,
      instagram_caption: result.instagram_caption,
      facebook_caption: result.facebook_caption,
      carousel_summary: result.carousel_summary,
    }) ??
    coercePostText(result.slides) ??
    coercePostText(result.carousel) ??
    coercePostText(result.draft);

  if (post) result.post = post;

  if (typeof result.caveat === "string") {
    result.caveat = result.caveat.trim() || undefined;
  }
  if (typeof result.notes === "string") {
    result.notes = result.notes.trim() || undefined;
  }
