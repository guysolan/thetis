...
const PackageSummary = ({ index }: { index: number }) => {
    ...
    const selectedItemView = itemsView?.find((item) => String(item.item_id) === String(selectedItem?.item_id));

    if (!selectedItemView) return <p>No item selected</p>;

    return (
        <div className='flex flex-col gap-y-2'>
            <p>{form.watch(`order_items.${index}.quantity_change`)} x {selectedItemView?.item_name}</p>
